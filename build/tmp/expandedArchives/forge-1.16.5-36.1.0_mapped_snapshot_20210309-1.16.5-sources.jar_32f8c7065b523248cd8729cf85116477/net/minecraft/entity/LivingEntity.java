...
public abstract class LivingEntity extends Entity {
   ...
   public void baseTick() {
      ...
      if (this.isAlive()) {
         if (this.areEyesInFluid(FluidTags.WATER) && !this.world.getBlockState(new BlockPos(this.getPosX(), this.getPosYEye(), this.getPosZ())).matchesBlock(Blocks.BUBBLE_COLUMN)) {
   ...
   public double getVisibilityMultiplier(@Nullable Entity lookingEntity) {
      ...
      d0 = net.minecraftforge.common.ForgeHooks.getEntityVisibilityMultiplier(this, lookingEntity, d0);
      return d0;
   }
   ...
   protected void spawnDrops(DamageSource damageSourceIn) {
      ...
      if (this.isAdult() && this.world.getGameRules().getBoolean(GameRules.DO_MOB_LOOT)) {
   ...
   private void func_233628_a_(Entity p_233628_1_) {
      ...
      if (!p_233628_1_.removed && !this.world.getBlockState(p_233628_1_.getPosition()).getBlock().isIn(BlockTags.PORTALS)) {
         vector3d = p_233628_1_.getDismountPosition(this);
      } else {
   ...
   protected void handleFluidSneak() {
      this.setMotion(this.getMotion().add(0.0D, (double)-0.04F * this.getAttribute(net.minecraftforge.common.ForgeMod.SWIM_SPEED.get()).getValue(), 0.0D));
   }
   ...
   private Vector3d handleOnClimbable(Vector3d p_213362_1_) {
      if (this.isOnLadder()) {
         ...
         if (d2 < 0.0D && !this.getBlockState().isScaffolding(this) && this.hasStoppedClimbing() && this instanceof PlayerEntity) {
            d2 = 0.0D;
         }
   ...
   public void livingTick() {
      ...
      if (this.isJumping && this.func_241208_cS_()) {
         ...
         double d8 = this.getFluidJumpHeight();
   ...
   private void addItemParticles(ItemStack stack, int count) {
      for(int i = 0; i < count; ++i) {
         ...
         if (this.world instanceof ServerWorld) //Forge: Fix MC-2518 spawnParticle is nooped on server, need to use server specific variant
             ((ServerWorld)this.world).spawnParticle(new ItemParticleData(ParticleTypes.ITEM, stack), vector3d1.x, vector3d1.y, vector3d1.z, 1, vector3d.x, vector3d.y + 0.05D, vector3d.z, 0.0D);
         else
