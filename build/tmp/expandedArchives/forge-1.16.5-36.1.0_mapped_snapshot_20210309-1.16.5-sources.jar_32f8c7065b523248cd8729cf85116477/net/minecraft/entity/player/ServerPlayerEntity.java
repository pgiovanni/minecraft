...
public class ServerPlayerEntity extends PlayerEntity implements IContainerListener {
   ...
   public Entity changeDimension(ServerWorld server, net.minecraftforge.common.util.ITeleporter teleporter) {
      if (!net.minecraftforge.common.ForgeHooks.onTravelToDimension(this, server.getDimensionKey())) return null;
      ...
      if (registrykey == World.THE_END && server.getDimensionKey() == World.OVERWORLD && teleporter.isVanilla()) { //Forge: Fix non-vanilla teleporters triggering end credits
         ...
         if (portalinfo != null) {
            ...
            } else if (spawnPortal && server.getDimensionKey() == World.THE_END) {
            ...
            net.minecraftforge.fml.hooks.BasicEventHooks.firePlayerChangedDimensionEvent(this, registrykey, server.getDimensionKey());
         }
   ...
   protected Optional<TeleportationRepositioner.Result> func_241830_a(ServerWorld p_241830_1_, BlockPos p_241830_2_, boolean p_241830_3_) {
      ...
      if (optional.isPresent()) {
         ...
         Direction.Axis direction$axis = this.world.getBlockState(this.entityBlockPosition).func_235903_d_(NetherPortalBlock.AXIS).orElse(Direction.Axis.X);
   ...
   public void copyFrom(ServerPlayerEntity that, boolean keepEverything) {
      if (keepEverything) {
         ...
         this.entityBlockPosition = that.entityBlockPosition;
   ...
   public void setGameType(GameType gameType) {
      if (!net.minecraftforge.common.ForgeHooks.onChangeGameMode(this, this.interactionManager.getGameType(), gameType)) return;
      this.interactionManager.setGameType(gameType);
      this.connection.sendPacket(new SChangeGameStatePacket(SChangeGameStatePacket.CHANGE_GAMETYPE, (float)gameType.getID()));
   ...
   public void teleport(ServerWorld newWorld, double x, double y, double z, float yaw, float pitch) {
      ...
      if (newWorld == this.world) {
      ...
      } else if (net.minecraftforge.common.ForgeHooks.onTravelToDimension(this, newWorld.getDimensionKey())) {
         ServerWorld serverworld = this.getServerWorld();
         IWorldInfo iworldinfo = newWorld.getWorldInfo();
         this.connection.sendPacket(new SRespawnPacket(newWorld.getDimensionType(), newWorld.getDimensionKey(), BiomeManager.getHashedSeed(newWorld.getSeed()), this.interactionManager.getGameType(), this.interactionManager.func_241815_c_(), newWorld.isDebug(), newWorld.isFlatWorld(), true));
         this.connection.sendPacket(new SServerDifficultyPacket(iworldinfo.getDifficulty(), iworldinfo.isDifficultyLocked()));
         ...
         net.minecraftforge.fml.hooks.BasicEventHooks.firePlayerChangedDimensionEvent(this, serverworld.getDimensionKey(), newWorld.getDimensionKey());
      }
